...
async fn draw_cell_coloured(
    ...
    let base_color = if let Some(steps) = path_map.get(&coordinates) {
        ...
        let brightness = 1.0 - normalized + increased_brightness;
        Color::new(
        ...
        Color::new(
            random_colour.r * increased_brightness,
            random_colour.g * increased_brightness,
            random_colour.b * increased_brightness,
            1.0,
        )
    };
    if cell.walls.len() == 4 {
